...
def monomialDegree {s : Type*} (t : s →₀ ℕ) : ℕ :=
  t.Sum fun _ e => e
...
theorem monomialDegree_le_of_le {σ : Type*} {m m' : σ →₀ ℕ} (h : m' ≤ m) :
    monomialDegree m' ≤ monomialDegree m :=
  Finsupp.sum_le_sum h

theorem monomialDegree_add {σ : Type*} (m m' : σ →₀ ℕ) :
...
theorem monomial_lemma_2 {σ : Type*} {m m' a : σ →₀ ℕ} (c : a - m ≤ m') : a ≤ m' + m :=
  tsub_le_iff_right.1 c

theorem monomial_lemma_3 {σ : Type*} {m m' : σ →₀ ℕ} : m ≤ m - m' + m' :=
  monomial_lemma_2 (le_refl _)

theorem monomial_lemma_1 {σ : Type*} {m m' a : σ →₀ ℕ} (h_m_le_a : m ≤ a) (c : a - m ≤ m') :
    m' - (a - m) = m' + m - a := by
  ext i
...
theorem totalDegree_monomial_eq_monomialDegree {σ R : Type*} [CommSemiring R] {m : σ →₀ ℕ} {a : R}
    (h : a ≠ 0) : totalDegree (monomial m a) = monomialDegree m := by
  convert total_degree_monomial m h

-- Use monomial instead of single!
theorem monomialDegree_single {σ : Type*} {j : σ} {d : ℕ} : monomialDegree (single j d) = d := by
...
theorem monomialDegree_le_totalDegree {σ R : Type*} [CommSemiring R] {m : σ →₀ ℕ}
...
theorem le_totalDegree {R σ : Type*} [CommSemiring R] {i : σ} {p : MvPolynomial σ R} {m : σ →₀ ℕ}
...
def MaxDegreeMonomial {R : Type*} [CommSemiring R] (t : σ →₀ ℕ) (f : MvPolynomial σ R) : Prop :=
  t ∈ f.support ∧ monomialDegree t = totalDegree f

-- this uses a lemma from flt-regular
theorem support_nonempty_iff {R σ : Type*} [CommSemiring R] {f : MvPolynomial σ R} :
...
def DominantMonomial {R : Type*} [CommSemiring R] (t : σ →₀ ℕ) (f : MvPolynomial σ R) : Prop :=
  MaxDegreeMonomial t f ∧ ∀ t' : σ →₀ ℕ, MaxDegreeMonomial t' f → t' = t

theorem dominantMonomial_of_factor_is_factor_of_maxDegreeMonomial {R : Type*} [CommRing R]
...
theorem totalDegree_lt_iff {R σ : Type*} [CommSemiring R] {f : MvPolynomial σ R} {d : ℕ}
    (h : 0 < d) : totalDegree f < d ↔ ∀ m : σ →₀ ℕ, m ∈ f.support → monomialDegree m < d := by
  rwa [total_degree_eq', Finset.sup_lt_iff]

theorem totalDegree_sub_lt {R σ : Type*} [CommRing R] [IsDomain R] {f g : MvPolynomial σ R} {k : ℕ}
...
theorem maxDegreeMonomialIffOfEqDegree' {R σ : Type*} [CommSemiring R] (p : MvPolynomial σ R)
...
theorem maxDegreeMonomial_iff_of_eq_degree {R σ : Type*} [CommSemiring R] (p : MvPolynomial σ R)
...
theorem maxDegreeMonomial_iff {R σ : Type*} [CommRing R] {f : MvPolynomial σ R} {m : σ →₀ ℕ} :
...
theorem dominantMonomial_iff {R σ : Type*} [CommRing R] {f : MvPolynomial σ R} {m : σ →₀ ℕ} :
...
theorem induction_on_totalDegree {R σ : Type*} [CommSemiring R] {M : MvPolynomial σ R → Prop}
    (p : MvPolynomial σ R)
...
theorem induction_on_new {R σ : Type*} [CommSemiring R] {M : MvPolynomial σ R → Prop}
    (p : MvPolynomial σ R)
    (h_add_weak :
      ∀ (a : σ →₀ ℕ) (b : R) (f : (σ →₀ ℕ) →₀ R),
