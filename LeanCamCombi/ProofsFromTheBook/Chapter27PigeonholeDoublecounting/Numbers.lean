...
theorem ineq_tec (n : ℕ) : 2 * n < 2 ^ (n + 1) := by induction' n with n ih <;> omega
...
theorem decompo_lemma (n a : ℕ) (aR : a ∈ Icc 1 (2 * n)) :
  ...
  let facSet : Finset ℕ := {q ∈ range (n + 1) | ∃ p, a = 2 ^ q * p}
  ...
  by_contra! con
  rw [Ne, Nat.mod_two_ne_one] at con
...
theorem size_lemma (n : Nat) : ((Icc 1 (2 * n)).filter fun x => x % 2 = 1).card = n := by
  ...
  · intro i j idef jdef eq
    simp only [Nat.one_ne_zero, add_left_inj, mul_eq_mul_left_iff, or_false, two_ne_zero] at eq
    exact eq
...
theorem claim_2 (n : Nat) (hn : 1 ≤ n) (A : Finset ℕ)
  ...
  · use a; use adef; use b; use bdef
    ...
    rw [← mul_assoc]
    rw [← pow_add]
    -- rw add_sub_cancel, -- we're not in a group
