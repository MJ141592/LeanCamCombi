...
@[protect_proj]
class IsRootSystem (k : Type*) {V : Type*} [CommRing k] [CharZero k] [AddCommGroup V] [Module k V]
    (Φ : Set V) : Prop where
  Finite : Φ.Finite
...
variable {k V : Type*} [CommRing k] [CharZero k] [AddCommGroup V] [Module k V]
...
def isRootSystemEquiv {V₂ : Type*} [AddCommGroup V₂] [Module k V₂] {Φ₂ : Set V₂}
    (h₂ : IsRootSystem k Φ₂) :=
  {e : V ≃ₗ[k] V₂ | e '' Φ = Φ₂}

theorem symm_equiv {α β : Type*} (f : α ≃ β) (s : Set α) (d : Set β) (h : f '' s = d) :
    f.symm '' d = s := by
  rw [← h]
  rw [Equiv.symm_image_image]

theorem symm_root_system_equiv {V₂ : Type*} [AddCommGroup V₂] [Module k V₂]
...
theorem finite_symmetries : Finite h.symmetries := by-- Should follow from `module.finite_stabilizer_of_finite_of_span_eq_top`
...
example (G : Type*) [Group G] (H : Subgroup G) [Finite G] : Finite H := by
