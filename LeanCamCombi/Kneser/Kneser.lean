...
theorem mul_kneser :
  ...
  let C := argminOn (fun C : Finset G ↦ #(Stab C)) IsWellFounded.wf _ convergent_nonempty
  set H := Stab C with hH
  obtain ⟨hCst, hCcard⟩ : C ∈ convergent := argminOn_mem _ _ _ _
  have hCmin : ∀ D : Finset G, D.mulStab ⊂ H → ¬D ∈ convergent := fun D hDH hD =>
    (card_lt_card hDH).not_le $ argminOn_le (fun D : Finset G => #D.mulStab) _ _ hD
  clear_value C
