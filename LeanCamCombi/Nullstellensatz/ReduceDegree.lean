...
def IsReduced {R σ : Type*} [CommRing R] (f : MvPolynomial σ R) (m : σ →₀ ℕ) : Prop :=
  ∀ m' ∈ f.support, ¬m ≤ m'
...
private def M {R σ τ : Type*} [CommRing R] [IsDomain R] [Fintype τ] {g : τ → MvPolynomial σ R}
...
private theorem h_add_weak_aux_comp {R σ τ : Type*} [CommRing R] [Fintype τ]
...
private theorem reduce_degree_h_add_weak {R σ τ : Type*} [CommRing R] [IsDomain R] [Fintype τ]
...
private theorem total_degree_p_aux {σ : Type*} {m m' a : σ →₀ ℕ} (h_m_le_a : m ≤ a)
...
private theorem total_degree_p {R σ : Type*} [CommRing R] [IsDomain R] {g : MvPolynomial σ R}
...
private theorem reduce_degree_h_monomial_a_eq_zero {R σ τ : Type*} [CommRing R] [IsDomain R]
...
private theorem reduce_degree_h_monomial_comp {R σ τ : Type*} [CommRing R] [IsDomain R] [Fintype τ]
...
private theorem reduce_degree_h_monomial {R σ τ : Type*} [CommRing R] [IsDomain R] [Fintype τ]
    {g : τ → MvPolynomial σ R} {m : τ → σ →₀ ℕ} (hm : ∀ i : τ, DominantMonomial (m i) (g i))
    (h0 : ∀ i : τ, 0 < totalDegree (g i)) (hmonic : ∀ i : τ, coeff (m i) (g i) = 1) (a : σ →₀ ℕ)
  ...
  · use fun i => 0
    simp only [true_and, MulZeroClass.zero_mul, imp_true_iff, true_or_iff, eq_self_iff_true,
...
theorem reduce_degree {R σ τ : Type*} [CommRing R] [IsDomain R] [Fintype τ] (f : MvPolynomial σ R)
    (g : τ → MvPolynomial σ R) {m : τ → σ →₀ ℕ} (hm : ∀ i : τ, DominantMonomial (m i) (g i))
    (h0 : ∀ i : τ, 0 < totalDegree (g i)) (hmonic : ∀ i : τ, coeff (m i) (g i) = 1) :
    ∃ h : τ → MvPolynomial σ R,
      (∀ i : τ, h i = 0 ∨ totalDegree (h i) + totalDegree (g i) ≤ totalDegree f) ∧
...
theorem reduce_degree' {R σ : Type*} [CommRing R] [IsDomain R] [Fintype σ] (f : MvPolynomial σ R)
...
theorem reduce_degree_special_case {R σ : Type*} [CommRing R] [IsDomain R] [Fintype σ]
