...
def main():
    ...
    for pr in pr_dict:
        for file in pr_dict[pr]["files"]:
            pr_data = {"number" : pr, "title" : pr_dict[pr]["title"]}
    ...
    with open("./docs/_includes/ready_to_upstream.md", 'w+') as writer:
        text = ""
    ...
    with open("./docs/_includes/easy_to_unlock.md", 'w+') as writer:
        text = ""
        for file_path in project_files:
            if project_files[file_path]["num_sorries"] == 0: continue
            if project_files[file_path]["depends"]: continue
