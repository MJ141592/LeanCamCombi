...
theorem c_mem_supported {R σ : Type*} [CommSemiring R] (s : Set σ) (a : R) : C a ∈ supported R s :=
  mem_supported.2 (by simp)

theorem support_monomial_of_mem_support_of_supported {R σ : Type*} [CommSemiring R]
...
theorem eq_single_of_mem_support_of_supported_singleton {R σ : Type*} [CommSemiring R]
...
theorem single_totalDegree_mem_support_of_supported_singleton {R σ : Type*} [CommSemiring R]
...
theorem dominantMonomialSingleOfSupportedSingleton {R σ : Type*} [CommSemiring R]
    {p : MvPolynomial σ R} {i : σ} (h : p ≠ 0) (hp : p ∈ supported R ({i} : Set σ)) :
...
theorem x_sub_c_ne_0 {R σ : Type*} [CommRing R] [DecidableEq σ] [Nontrivial R] (i : σ) (a : R) :
    X i - C a ≠ 0 := by
...
theorem eval_g_S_eq_zero {R σ : Type*} [CommRing R] [IsDomain R] (S : Finset R) (hS : 0 < S.card)
    (s : σ → R) (i : σ) (h_s : s i ∈ S) : eval s (∏ s in S, (X i - C s)) = 0 := by
  simp [eval_prod, Finset.prod_eq_zero h_s]

theorem g_S_ne_0 {R σ : Type*} [CommRing R] [IsDomain R] [DecidableEq σ] (S : Finset R) (i : σ) :
    ∏ s in S, (X i - C s) ≠ 0 := by
