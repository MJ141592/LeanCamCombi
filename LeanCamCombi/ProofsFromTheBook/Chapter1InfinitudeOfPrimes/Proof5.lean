...
theorem n_closed (a b : ℤ) (b_tec : 0 < b) : IsClosed (n a b) := by
  rw [← isOpen_compl_iff]
  simp_rw [n_as_a_complement a b b_tec, ← Finset.mem_coe]
...
theorem two_units_not_open : ¬bonaFideTopology.IsOpen {(-1 : ℤ), 1} := by
  ...
  obtain con | con := con
  · have problem : (-1 : ℤ) ∈ (∅ : Set ℤ) := by rw [← con]; apply Set.mem_insert
...
theorem univ_sdiff_units_as_prime_union : {(-1 : ℤ), (1 : ℤ)}ᶜ = ⋃ p ∈ {x : ℕ | x.Prime}, n 0 p := by
  ...
  · simp [n, ← Int.natCast_one, ← not_or, ← Int.natAbs_eq_iff, or_comm, -Nat.cast_one,
      -Int.Nat.cast_ofNat_Int, -Int.ofNat_one]
    rintro p p_prime d rfl
    simp [Int.natAbs_mul, p_prime.ne_one]
