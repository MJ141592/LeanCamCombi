...
@[simp]
theorem Module.coe_end_one {k V : Type*} [Semiring k] [AddCommMonoid V] [Module k V] :
    ⇑(1 : (Module.End k V)ˣ) = id :=
  rfl

#print LinearEquiv.coe_one /-
@[simp]
theorem LinearEquiv.coe_one {k V : Type*} [Semiring k] [AddCommMonoid V] [Module k V] :
    ⇑(1 : V ≃ₗ[k] V) = id :=
  rfl
-/
...
theorem toPreSymmetry_apply_self {x : V} {f : Dual k V} (h : f x = 2) : toPreSymmetry x f x = -x := by rw [toPreSymmetry_apply, h, ← one_smul k x, smul_smul, ← sub_smul]; norm_num
...
theorem eq_dual_of_toPreSymmetry_image_subseteq [CharZero k] [NoZeroSMulDivisors k V] {x : V}
  ...
  suffices IsOfFinOrder u by
    ...
    simpa only [hn₁, smul_eq_zero, Nat.cast_eq_zero, hn₀.ne', false_or_iff, or_false, hx,
...
@[simp]
theorem nontrivial_dual_iff {k V : Type*} [Field k] [AddCommGroup V] [Module k V] :
...
below trivial. -/
def averageBilinear {G : Type*} [Group G] [Finite G] (ρ : G →* V ≃ₗ[k] V) (B : V →ₗ[k] Dual k V) :
    V →ₗ[k] Dual k V
    where
...
@[simp]
theorem QuadraticForm.comp_posDef_iff {k V : Type*} [LinearOrderedField k] [AddCommGroup V]
...
theorem averageBilinear_eq_sum {G : Type*} [Group G] [Finite G] (ρ : G →* V ≃ₗ[k] V)
    (B : V →ₗ[k] Dual k V) :
...
-- TODO Should be via `simp`
@[simp]
theorem averageBilinear_smul_smul {G : Type*} [Group G] [Finite G] (ρ : G →* V ≃ₗ[k] V)
...
@[simp]
theorem Basis.toDual_total_left' {R M ι : Type*} [CommSemiring R] [AddCommMonoid M] [Module R M]
    [DecidableEq ι] (b : Basis ι R M) (f : ι →₀ R) : b.toDual (Finsupp.total ι M R b f) ∘ b = f := by ext i; simp only [Function.comp_apply]; simp
...
theorem QuadraticForm.PosDef.sum {k V ι : Type*} [Finite ι] [Nonempty ι] [LinearOrderedField k]
...
/-- The assumption `linear_ordered_field` is stronger than necessary but enables an easy proof by just taking the average of a positive definite bilinear form. -/
theorem exists_to_dual_ker_eq_bot {k V G : Type*} [LinearOrderedField k] [AddCommGroup V]
...
variable {k V : Type*} [Field k] [AddCommGroup V] [Module k V] {p : Submodule k V}
