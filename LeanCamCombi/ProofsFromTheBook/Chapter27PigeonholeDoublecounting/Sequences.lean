...
theorem lengths_nonempty (I : Finset ℕ) (a : ℕ → ℝ) (i : ℕ) (i_I : i ∈ I) :
    ((range (I.card + 1)).filter fun t =>
        ∃ J ⊆ I, J.card = t ∧ IsIncreasingSubseq J a i).Nonempty := by
  use 1
...
noncomputable def longestIncrSubseqLen (I : Finset ℕ) (a : ℕ → ℝ) (i : ℕ) (i_I : i ∈ I) : ℕ :=
  Finset.max'
    ((range (I.card + 1)).filter fun t =>
      ∃ J ⊆ I, J.card = t ∧ IsIncreasingSubseq J a i)
...
theorem lisl_non_extendable (I : Finset ℕ) (a : ℕ → ℝ) (i j : ℕ) (i_I : i ∈ I) (j_I : j ∈ I)
  ...
  have tec1 :
    longestIncrSubseqLen I a i i_I ∈
      (range (I.card + 1)).filter fun t =>
        ∃ J ⊆ I, J.card = t ∧ IsIncreasingSubseq J a i := by
  ...
  have extend : IsIncreasingSubseq (insert j index_seq) a j := by
    ...
    · intro x xdef y ydef xly
      ...
      · specialize index_seq_imin y y_index
        ...
        obtain index_seq_imin | index_seq_imin := index_seq_imin
        rw [← index_seq_imin]
        exact con
  ...
  have contra_pre :
    (insert j index_seq).card ∈
      (range (I.card + 1)).filter fun t =>
        ∃ J ⊆ I, J.card = t ∧ IsIncreasingSubseq J a j := by
    rw [mem_filter]
    constructor
    · rw [mem_range]
...
theorem erdos_szekeres (n m : ℕ) (n_pos : 0 < n) (m_pos : 0 < m) (a : ℕ → ℝ)
    ...
    (∃ i ∈ range (m * n + 1),
        ∃ I ⊆ range (m * n + 1), IsIncreasingSubseq I a i ∧ I.card ≥ m + 1) ∨
      ∃ i ∈ range (m * n + 1),
        ∃ I ⊆ range (m * n + 1), IsDecreasingSubseq I a i ∧ I.card ≥ n + 1 := by
