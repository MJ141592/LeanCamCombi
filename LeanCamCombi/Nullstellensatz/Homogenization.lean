...
theorem Finsupp.sum_update_add {α β : Type*} [AddCommMonoid α] [AddCommMonoid β] (f : ι →₀ α)
...
@[simp]
theorem support_mapDomain {α β M : Type*} [AddCommMonoid M] (f : α ↪ β) (v : α →₀ M) :
...
theorem mapDomain_apply' {α β M : Type*} [AddCommMonoid M] (S : Set α) {f : α → β} (x : α →₀ M)
...
theorem mapDomain_injOn {α β M : Type*} [AddCommMonoid M] (S : Set α) {f : α → β}
    (hf : Set.InjOn f S) :
  ...
  have :
    ...
    have :
      (t + Finsupp.single i (p.total_degree - t.sum fun _ m => m)) a =
        (y + Finsupp.single i (p.total_degree - y.sum fun _ m => m)) a := by rw [hh]
...
@[simp]
theorem homogenization_x_sub_c {R : Type*} [CommRing R] {i j : ι} (r : R) :
    (X j - C r : MvPolynomial ι R).homogenization i = X j - C r * X i := by
...
@[simp]
theorem homogenization_x_pow_sub_c {R : Type*} [CommRing R] {i j : ι} {n : ℕ} (hn : 0 < n)
...
theorem homogenization_prod {σ S : Type*} [CommRing S] [IsDomain S] (i : ι)
...
theorem homogenization_prod_id {S : Type*} [CommRing S] [IsDomain S] (i : ι)
