...
- `claim` :
      ...
      upperbounded by the harmonic sum up to n, and lowerbounded by its predecessor.
...
-/
theorem num_of_mult_le (n m : ℕ) (hm : m ≠ 0) :
  ...
  exact eq.resolve_right hm

open scoped Classical in
/-- Expressing the mulitples of `m` in the fromework of
`finset.bipartiteAbove`, for double counting.
-/
theorem mult_rel (n m : ℕ) :
...
`finset.bipartiteBelow`, for double counting.
-/
theorem dvd_rel (n m : ℕ)  :
...
-/
theorem main_result_pre (n : ℕ) :
    ∑ m in Icc 1 n, ((Icc 1 n).filter fun x => ∃ q, x = m * q).card =
      ∑ m in Icc 1 n, ((Icc 1 n).filter fun x => ∃ q, m = x * q).card := by apply sum_card_bipartiteAbove_eq_sum_card_bipartiteBelow
...
theorem lb_pre (n m : ℕ) (hm : m ≠ 0) : (n / m : ℚ) - 1 ≤ ((n / m : ℕ) : ℚ) := by
  ...
  rw [show (m : ℚ) * (n / m : ℚ) = (n : ℚ) by
      ...
      intro con
      rw [show (0 : ℚ) = ↑(0 : ℕ) by simp] at con
      rw [Nat.cast_inj] at con
      exact hm con]
  rw [show (m : ℚ) * ↑(n / m : ℕ) = ↑(m * (n / m : ℕ)) by
      simp only [mul_eq_mul_left_iff, true_or, eq_self_iff_true, Nat.cast_inj, Nat.cast_eq_zero,
        Nat.cast_mul]]
...
theorem lowerbound (n : ℕ) (hn : n ≠ 0) :
  ...
  simp [one_div, nsmul_eq_mul, Nat.cast_add, Finset.sum_congr] at pre
  -- technique use simp look at the simp only [] and delete the undesired simplifcations from the list
  simp [one_div, nsmul_eq_mul, Nat.cast_add, Finset.sum_congr]
  rw [show (n : ℚ) * (↑n)⁻¹ = 1 by
